...
void TCP::establish_connection()
{
    ...
    {
        ESP_LOGE(TRANSMISSION_TAG, "couldn't get hostname for `%s` "
    ...
    ESP_LOGI(TRANSMISSION_TAG, "Socket created, connecting to %s:%s", TCP_CLIENT_CONNECT_ADDRESS, TCP_CLIENT_CONNECT_PORT);

    // Marking the socket as non-blocking
    int flags = fcntl(sock, F_GETFL);
    if (fcntl(sock, F_SETFL, flags | O_NONBLOCK) == -1)
    {
        log_socket_error(TRANSMISSION_TAG, sock, errno, "Unable to set socket non blocking");
    }
    ...
    if (connect(sock, address_info->ai_addr, address_info->ai_addrlen) != 0)
    {
        if (errno == EINPROGRESS)
        {
            ...
            if (res < 0)
            {
                log_socket_error(TRANSMISSION_TAG, sock, errno, "Error during connection: select for socket to be writable");
                socket_error_handling(sock, *address_info);
            }
            else if (res == 0)
            {
                log_socket_error(TRANSMISSION_TAG, sock, errno, "Connection timeout: select for socket to be writable");
                socket_error_handling(sock, *address_info);
            }
            else
            {
                ...
                {
                    log_socket_error(TRANSMISSION_TAG, sock, errno, "Error when getting socket error using getsockopt()");
                    socket_error_handling(sock, *address_info);
                }
                if (sockerr)
                {
        ...
        {
            log_socket_error(TRANSMISSION_TAG, sock, errno, "Socket is unable to connect");
            socket_error_handling(sock, *address_info);
        }
    }
}

const char *TCP::get_data()
{
    PACKET_DATA initial_payload = packet_to_send;

    std::byte aux[sizeof(PACKET_DATA) - 1] = {};

    memcpy(&aux, &initial_payload, sizeof(PACKET_DATA) - 1); // can we fucked up with padding?
...
void TCP::send_data(const char *payload)
{

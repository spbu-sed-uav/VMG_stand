...
// float thermistor::NTC_compute(float analog, float baseDiv, uint16_t B, uint8_t t, uint8_t res) {
...
// float thermistor::NTC_compute(float analog, uint32_t R, uint16_t B, uint8_t t, uint32_t Rt, uint8_t res) {
//     return NTC_compute(analog, (float)R / Rt, B, t, res);
// }

//     // пин, R резистора, B термистора, t термистора, R термистора, разрешение АЦП
//     thermistor::setup_thermistor(uint8_t pin, uint32_t R, uint16_t B, uint8_t t = 25, uint32_t Rt = 10000, uint8_t res = 10)
//     {
//         config(R, B, t, Rt);
//         setPin(pin, res);
//     }
...
//     void thermistor::config(uint32_t R, uint16_t B, uint8_t t = 25, uint32_t Rt = 10000)
//     {
//         _beta = B;
//         _tempBase = t;
...
//     float thermistor::getTempAverage(uint8_t samples = 20)
//     {
//         uint16_t aver = 0;
